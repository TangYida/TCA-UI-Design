...
- [x] 新增全局 Sign In／Sign Up 弹窗：桌面与移动导航、评论区登录提示以及 Free Registration 共用同一入口；两种状态统一以 Sign In 为视觉基准。弹窗卡片按完整 Sign Up 内容固定为 `600px` 高，Submit 保持在卡片内部；外层改为 `100dvh` 全视口网格居中，移动端上下等距留白，并避免原生 `dialog` 的移动端定位偏差及模式切换跳动。
